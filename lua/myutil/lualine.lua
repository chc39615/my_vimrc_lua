...
function M.cmp_source(name, icon)
	...
	local colors = {
		ok = Myutil.ui.fg("Special"),
		error = Myutil.ui.fg("DiagnosticError"),
		pending = Myutil.ui.fg("DiagnosticWarn"),
	}

	return {
		function()
			return icon or Myutil.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
		end,
		cond = function()
			return status() ~= nil
		end,
...
function M.pretty_path(opts)
	...
	return function(self)
		...
		local root = Myutil.root.get({ normalize = true })
		local cwd = Myutil.root.cwd()
