...
function M.rename_file()
	...
	local old = assert(Myutil.root.realpath(vim.api.nvim_buf_get_name(buf)))
	local root = assert(Myutil.root.realpath(Myutil.root.get({ normalize = true })))
...
function M.formatter(opts)
	...
	return Myutil.merge(ret, opts) --[[@as LazyFormatter]]
end
			...
			callback = function(ev)
				...
				if not ({ M.words.get() })[2] then
					if ev.event:find("CursorMoved") then
						vim.lsp.buf.clear_references()
					elseif not Myutil.cmp.visible() then
